...
def is_sol(sol, max_ego):
    return bool(sol['selected']) and (sol['ego'] - sol['max_ego']) < max_ego
...
def bt(players, sol, best_sol, max_ego, k):
    ...
    for i in range(k, len(players)):
        o = players[i]
        prev_max = sol['max_ego']
        sol['selected'].add(o)
        sol['ego'] += o[1]
        sol['max_ego'] = max(sol['max_ego'], o[1])
        sol['skill'] += o[2]
        best_sol = bt(players, sol, best_sol, max_ego, i + 1)
        sol['selected'].remove(o)
        sol['ego'] -= o[1]
        sol['max_ego'] = prev_max
        sol['skill'] -= o[2]
    return best_sol
